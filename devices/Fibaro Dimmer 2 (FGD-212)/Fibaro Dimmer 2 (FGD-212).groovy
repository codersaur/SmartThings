 ...
 *
 *  Fibaro Dimmer 2 (FGD-212) physical device notes:
 *   - The auto-calibration feature will sometimes set parameter 1 too high. Use GetConfig to find out what values
 *     have been set by auto-calibration and adjust as necessary.
 *   - Sending level 0, will switch the dimmer off and does NOT change the level.
 *
 *  Version History:
 *
 *   2016-10-11: v1.01
 ...
 *    - on(), off(), setLevel(): Delayed switchMultilevelGet() requests by 8s as early requests generate erroneous
 *      data. The dimmer will send a correct report once it has completed the request anyway (which is usually sooner than 8s).
 *    - zwaveEvent(_MeterReport_): Removed rounding of power values. Also, Energy and power values are stored 
 *      as dispEnergy and dispPower to work-around UI formatting issue.
 ...
 *      [On Hold: This is not currently possible due to ST platform limitations. Device preferences are read-only!]
 *
 *  License:
...
metadata {
	definition (name: "Fibaro Dimmer 2", namespace: "codersaur", author: "David Lomas") {
        ...
        fingerprint deviceId: "0x1001", inClusters: "0x5E, 0x20, 0x86, 0x26, 0x85, 0x59, 0x8E, 0x32, 0x31, 0x71"
	}
	...
	tiles(scale: 2) {
		...
		details(["switch","instMode","power","nightmode","lastReset","energy","configureAfterSecure","getConfig","refresh"])
	}
    ...
    preferences {
    	section { // NIGHTMODE:
	        ...
	        input name: "configNightmodeLevel", type: "number", range: "1..99", defaultValue: "10", required: true,
 	           title: paragraph + "\n\n" +
                   "1. Nightmode Level: The dimmer will always switch on at this level during nightmode."
        
			input name: "configNightmodeForce", type: "boolean", defaultValue: true, required: true,
        ...
        section { // GENERAL:
			// Debug Mode:
			input name: "configDebugMode", type: "boolean", title: "GENERAL:\n\nEnable debug logging?", defaultValue: false, required: false
        }
        
        section { // DEVICE PARAMETERS:
        
			def paragraph = "DEVICE PARAMETERS:\n\nGROUP 0 - The Dimmer 2 behavior - Basic functionalities"
			...
			input name: "param24", type: "number", range: "0..31", defaultValue: "0", required: true,
				 title: "24. Command frames sent in 2nd and 3rd association groups (S1 associations). " +
						...
						"Available settings (add together):\n" +
			...
			input name: "param27", type: "number", range: "0..15", defaultValue: "15", required: true,
				 title: "27. Associations in Z-Wave network security mode. " +
						...
						"E.g. 15 = all groups (II-V) sent as secure.\n" +
						"Default value: 15."
...
def parse(String description) {
	if (state.debug) log.debug "${device.displayName}: parse(): Raw command: ${description}"
	def result = null

    if (description != "updated") {
		// zwave.parse: The second parameter specifies which command version to return for each command type:
    	// FGD-212 supports:
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd) {
	log.trace "${device.displayName}: Switch Basic Report received: ${cmd}"
	dimmerEvents(cmd)
}

def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicSet cmd) {
	log.trace "${device.displayName}: Switch Basic Set received: ${cmd}"
	//dimmerEvents(cmd)
}
...
{
	log.trace "${device.displayName}: Switch Binary Report received: ${cmd}"
	dimmerEvents(cmd)
}
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd) {
	log.trace "${device.displayName}: Switch Multilevel Report received: ${cmd}"
	dimmerEvents(cmd)
}
 ...
 *
 *  Requests a Meter Report if there has been a change.
 **/
def dimmerEvents(physicalgraph.zwave.Command cmd) {
	...
	result << switchEvent
    isStateChange = switchEvent.isStateChange
    
	if (cmd.value) {
    	def levelEvent = createEvent(name: "level", value: cmd.value, unit: "%")
		result << levelEvent
        isStateChange = (switchEvent.isStateChange || levelEvent.isStateChange)
	}
    
	if (isStateChange) {
    	if (state.debug) log.debug "${device.displayName}: dimmerEvents(): State changed, so checking for pending level and requesting meter report."
        def pendingCmd = ((value == "on") ? checkNightmode() : []) // Only restore pending level if dimmer is switching on.
	    result << response(pendingCmd + ["delay 5000", secure(zwave.meterV3.meterGet(scale: 2)),"delay 10000", secure(zwave.meterV3.meterGet(scale: 2))])
...
def zwaveEvent(physicalgraph.zwave.commands.sceneactivationv1.SceneActivationSet cmd) {
	log.trace "${device.displayName}: Scene Activation: ${cmd}"
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
	log.trace "${device.displayName}: Meter Report received: ${cmd}"
	...
	if (cmd.meterType == 1) {
		if (cmd.scale == 0) {
        	...
        	// Accumulated Energy (kVAh) - Ignored
            //sendEvent(name: "dispEnergy", value: String.format("%.2f",cmd.scaledMeterValue as BigDecimal) + " kVAh", displayed: false)
	    	...
	    	// Instantaneous Voltage (Volts) - Not supported by the FGD-212.
	    	...
	    	// Instantaneous Current (Amps) - Not supported by the FGD-212.
	    	...
	    	// Instantaneous Power Factor - Not supported by the FGD-212.
            ...
            // Unknown Meter Report Scale:
            log.warn "${device.displayName}: zwaveEvent(): Unhandled Meter Report: Scale: ${cmd.scale}"
		}
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv1.ConfigurationReport cmd) {
	log.trace "${device.displayName}: Configuration Report received: ${cmd}"
    
    // Translate configurationValue (byte array) back to scaledConfigurationValue (double):
	...
	// Cache param19 as it's needed for nightmode:
	if ( 19 == cmd.parameterNumber.toInteger()) state.preferencesCache_param19 = scValue
}

/**
 *  securityv1.SecurityMessageEncapsulation (0x98)
 *
...
def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
	log.trace(cmd)
	def encapsulatedCommand = cmd.encapsulatedCommand([0x20: 1, 0x25: 1, 0x26: 3, 0x28: 1, 0x2B: 1, 0x30: 1, 0x32: 3, 0x70: 1, 0x72: 1, 0x73: 1, 0x85: 2, 0x90: 1, 0x98: 1, 0x9B: 1]) // can specify command class versions here like in zwave.parse
 ...
 **/
def poll() {
	log.trace("${device.displayName}: poll()")
	secureSequence([
		zwave.switchMultilevelV1.switchMultilevelGet(),
 ...
 **/
def refresh() {
	log.trace("${device.displayName}: refresh()")
	secureSequence([
    	zwave.switchMultilevelV1.switchMultilevelGet(),
		zwave.meterV3.meterGet(scale: 0),
		zwave.meterV3.meterGet(scale: 2)
 ...
 **/
def reset() {
	log.trace("${device.displayName}: reset()")
    
    state.lastReset = new Date().format("YYYY/MM/dd \n HH:mm:ss", location.timeZone)
    sendEvent(name: "lastReset", value: state.lastReset, unit: "",descriptionText: "Accumulated Energy Reset")
	state.energy = 0.00
...
/**
 *  configure() - Not sure when this is run?
 *
 **/
def configure() {
...
def updated() {
    ...
    state.preferencesCache_param19 = param19.toInteger()
...
/**
 *  manageSchedules() - Unschedule/schedule Nightmode Start and Stop.
 **/
def manageSchedules() {
	if (state.debug) log.debug "$device.displayName: manageSchedules()"
	
    if (configNightmodeStartTime) {
		if (state.debug) log.debug "$device.displayName: manageSchedules(): Nightmode scheduled to start at ${configNightmodeStartTime}"
    ...
    if (configNightmodeStopTime) {
		if (state.debug) log.debug "$device.displayName: manageSchedules(): Nightmode scheduled to stop at ${configNightmodeStopTime}"
...
def enableNightmode(level=-1) {
	log.info "${device.displayName}: enableNightmode(${level})"
    
    def cmds = []
    
    // Clean level value:
    if (level == -1) level = configNightmodeLevel
	if (level > 99) level = 99
    if (level < 1) level = 1
    
    // If nightmode is not already active, save current level and current value of param19, so they can be restored when nightmode is stopped:
    if (!state.nightmodeActive) {

	    state.nightmodePriorLevel = device.latestValue("level")
    	log.info  "${device.displayName}: enableNightmode(): Saved previous level: ${state.nightmodePriorLevel}"
    
    	if (!state.preferencesCache_param19) state.preferencesCache_param19 = 0
    	state.nightmodePriorParam19 = state.preferencesCache_param19.toInteger()
		log.info  "${device.displayName}: enableNightmode(): Saved previous param19: ${state.nightmodePriorParam19}"
    }
    
    // If the dimmer is already on, and configNightmodeForce is enabled, then adjust the level immediately:
    if (("on" == device.latestValue("switch")) & ("true" == configNightmodeForce)) cmds << secure(zwave.basicV1.basicSet(value: level))
    ...
    cmds << secure(zwave.configurationV1.configurationSet(parameterNumber: 19, size: 1, scaledConfigurationValue: level))
...
def disableNightmode() {
	log.info  "${device.displayName}: disableNightmode()"
    ...
    if (state.nightmodeActive) {
	    ...
	    if (state.debug) log.debug "${device.displayName}: disableNightmode(): Restoring previous value of param19 to: ${state.nightmodePriorParam19}"
        cmds << secure(zwave.configurationV1.configurationSet(parameterNumber: 19, size: 1, scaledConfigurationValue: state.nightmodePriorParam19.toInteger()))
		state.preferencesCache_param19 = state.nightmodePriorParam19
    	...
    	if (("on" == device.latestValue("switch")) & ("true" == configNightmodeForce)) {
        	...
        	if (state.debug) log.debug "${device.displayName}: disableNightmode(): Restoring level to: ${state.nightmodePriorLevel}"
    		cmds << secure(zwave.basicV1.basicSet(value: state.nightmodePriorLevel.toInteger()))
        } else if (0 == state.preferencesCache_param19) {
	        ...
	        if (state.debug) log.debug "${device.displayName}: disableNightmode(): Setting flag to restore level at next switch-on: ${state.nightmodePriorLevel}"
 ...
 *  checkNightmode()
 *
 *  Checks nightmodePendingLevel to see if there's a pending level which should be restored.
 **/
def checkNightmode() {
	if (state.debug) log.debug "$device.displayName: checkNightmode()"

	def cmds = []
    
	if (state.nightmodePendingLevel) {
    	if (state.debug) log.debug "$device.displayName: checkNightmode(): Applying Pending Level: ${state.nightmodePendingLevel}"
    	cmds = setLevel(state.nightmodePendingLevel)
        state.nightmodePendingLevel = false
    }
 ...
 **/
def toggleNightmode() {
	if (state.debug) log.debug "$device.displayName: toggleNightmode()"
...
/**
 *  changeSingleParamAfterSecure() - Change a single device parameter.
 **/
...
/**
 *  configureAfterSecure() - Set device parameters.
 **/
...
/**
 *  secure() - Encapsulate zwave commands.
 **/
...
/**
 *  secureSequence() - Secure and send an array of commands.
 **/
 ...
 **/
def test() {
	if (state.debug) log.debug "$device.displayName: Test()"
